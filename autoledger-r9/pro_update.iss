[Setup]
; R9 UPDATE package: same AppId/install/data/licence paths as R6/R8 so this is an in-place Pro update.
...
OutputDir=output
OutputBaseFilename=AUTOLEDGER_Pro_v2.2.5_R9_UPDATE_from_R6_TEST_Setup
Compression=lzma2
...
begin
  Result :=
    ...
    FileExists(ExpandConstant('{localappdata}\Programs\AUTOLEDGER Pro\AUTOLEDGER Pro v2.2.5 R9 TEST.exe'));
end;
...
begin
  ...
  begin
    if not WizardSilent then
      MsgBox('This is the AUTOLEDGER Pro R9 UPDATE package, but an existing R6/R8/R9 Pro installation was not found.' + #13#10 + #13#10 +
