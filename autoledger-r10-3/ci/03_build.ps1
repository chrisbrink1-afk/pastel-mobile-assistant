...
$p=Start-Process -FilePath $free -ArgumentList '--smoke-test' -PassThru -Wait; if($p.ExitCode-ne 0){throw "Free R10.3 smoke failed: $($p.ExitCode)"}
$p=Start-Process -FilePath $pro -ArgumentList '--smoke-test' -PassThru -Wait; if($p.ExitCode-ne 0){throw "Pro R10.3 smoke failed: $($p.ExitCode)"}
...
foreach($iss in @('free_update.iss','free_full.iss','pro_update.iss','pro_full.iss')){& $iscc $iss;if($LASTEXITCODE-ne 0){throw "Inno Setup failed: $iss"}}
Pop-Location
