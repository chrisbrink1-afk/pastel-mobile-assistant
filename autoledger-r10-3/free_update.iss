...
OutputDir=output
OutputBaseFilename=AUTOLEDGER_Free_v2.2.5_R10_3_UPDATE_from_OLDER_TEST_Setup
Compression=lzma2
SolidCompression=yes
WizardStyle=modern
LicenseFile=EULA.txt
SetupIconFile=assets\AUTOLEDGER_ICON.ico
UninstallDisplayIcon={app}\AUTOLEDGER Free v2.2.5 R10.3 TEST.exe
...
[Icons]
Name: "{group}\AUTOLEDGER Free"; Filename: "{app}\AUTOLEDGER Free v2.2.5 R10.3 TEST.exe"
Name: "{autodesktop}\AUTOLEDGER Free"; Filename: "{app}\AUTOLEDGER Free v2.2.5 R10.3 TEST.exe"
...
begin
  Result :=
    ...
    FileExists(ExpandConstant('{localappdata}\Programs\AUTOLEDGER Free\AUTOLEDGER Free v2.2.5 R10.3 TEST.exe'));
end;
...
begin
  ...
  begin
    if not WizardSilent then
      MsgBox('This is the AUTOLEDGER Free R10.3 UPDATE package, but an existing supported AUTOLEDGER Free installation was not found.' + #13#10 + #13#10 +
        'Please use the R10.3 FULL / Stand-alone installer on a new PC or clean installation.', mbError, MB_OK);
  end;
end;
...
begin
  ...
  begin
    ...
    SaveStringToFile(AddBackslash(DataDir) + 'install_context_r10.json',
      '{"revision":"R10.3","install_kind":"update"}', False);
  end;
end;
