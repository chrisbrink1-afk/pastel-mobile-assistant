...
OutputDir=output
OutputBaseFilename=AUTOLEDGER_Pro_v2.2.5_R10_UPDATE_from_OLDER_TEST_Setup
Compression=lzma2
SolidCompression=yes
WizardStyle=modern
LicenseFile=EULA.txt
SetupIconFile=assets\AUTOLEDGER_ICON.ico
UninstallDisplayIcon={app}\AUTOLEDGER Pro v2.2.5 R10 TEST.exe
...
[Icons]
Name: "{group}\AUTOLEDGER Pro"; Filename: "{app}\AUTOLEDGER Pro v2.2.5 R10 TEST.exe"
Name: "{autodesktop}\AUTOLEDGER Pro"; Filename: "{app}\AUTOLEDGER Pro v2.2.5 R10 TEST.exe"
...
begin
  Result :=
    ...
    FileExists(ExpandConstant('{localappdata}\Programs\AUTOLEDGER Pro\AUTOLEDGER Pro v2.2.5 R10 TEST.exe'));
end;
...
begin
  ...
  begin
    if not WizardSilent then
      MsgBox('This is the AUTOLEDGER Pro R10 UPDATE package, but an existing supported AUTOLEDGER Pro installation was not found.' + #13#10 + #13#10 +
        'Please use the R10 FULL / Stand-alone installer on a new PC or clean installation.', mbError, MB_OK);
  end;
end;
...
begin
  ...
  begin
    ...
    SaveStringToFile(AddBackslash(DataDir) + 'install_context_r10.json',
      '{"revision":"R10","install_kind":"update"}', False);
  end;
end;
