...
class Store:
    ...
    def _row_to_rule(r) -> Rule:
        direction = (r["direction"] if "direction" in r.keys() else "PAYMENT") or "PAYMENT"
        return Rule(r["id"], r["name"], r["mode"], r["pattern"], r["account"], bool(r["vat"]), int(r["tax_type"]), r["pastel_ref"], int(r["priority"]), bool(r["enabled"]), direction.upper())
    ...
    def save_rule(self, rule: Rule) -> int:
        ...
        if rule.id:
            self.conn.execute("""UPDATE rules SET name=?,mode=?,pattern=?,account=?,vat=?,tax_type=?,pastel_ref=?,priority=?,enabled=?,direction=?,updated_at=CURRENT_TIMESTAMP WHERE id=?""",
                              (rule.name, rule.mode, rule.pattern, rule.account, int(rule.vat), int(rule.tax_type), rule.pastel_ref, int(rule.priority), int(rule.enabled), direction, rule.id))
            rid = rule.id
        else:
            cur = self.conn.execute("""INSERT INTO rules(name,mode,pattern,account,vat,tax_type,pastel_ref,priority,enabled,direction) VALUES(?,?,?,?,?,?,?,?,?,?)""",
                                    (rule.name, rule.mode, rule.pattern, rule.account, int(rule.vat), int(rule.tax_type), rule.pastel_ref, int(rule.priority), int(rule.enabled), direction))
    ...
    def known_accounts(self) -> List[str]:
        rows = self.conn.execute("SELECT DISTINCT TRIM(account) AS account FROM rules WHERE TRIM(account) <> '' ORDER BY account").fetchall()
...
def apply_rules(txns: List[Transaction], rules: List[Rule], default_tax_type: int = 0):
    for t in txns:
        ...
        distinct = {(r.account, r.vat, r.tax_type) for r in top}
        ...
        t.status = f"Auto-assigned • recurring x{t.recurring_count}" if t.recurring_count >= 2 else "Auto-assigned"
...
def pastel_rows(txns: List[Transaction], settings: Dict[str, str]):
    ...
    for t in txns:
        ...
        desc = re.sub(r"\s+", " ", t.details).strip()[:36]
...
class RuleDialog(tk.Toplevel):
    ...
    def save(self):
        ...
        r = Rule(self.rule.id if self.rule else None, name, mode, pattern, account, self.vars["vat"].get(), tax_type, ref, priority, True, self.direction)
...
class ManualAssignDialog(tk.Toplevel):
    def __init__(self, parent, txn: Transaction, default_tax_type: str, known_accounts: Optional[List[str]] = None):
        ...
        ttk.Label(f,text="Pastel reference").grid(row=3,column=0,sticky="w",pady=4); ttk.Entry(f,textvariable=ref,width=40).grid(row=3,column=1,pady=4)
        vatframe=ttk.Frame(f); vatframe.grid(row=4,column=0,columnspan=2,sticky="w",pady=6)
        ...
        ttk.Label(f,text="This assignment applies only to this statement. Use 'Save rule' when you want future CSV files to remember the GL/VAT allocation.",wraplength=520).grid(row=5,column=0,columnspan=2,sticky="w",pady=(4,10))
        b=ttk.Frame(f); b.grid(row=6,column=0,columnspan=2,sticky="e")
        ttk.Button(b,text="Cancel",command=self.destroy).pack(side="right",padx=(8,0)); ttk.Button(b,text="Apply",command=lambda:self._save(a,v,tax,ref)).pack(side="right")
        self.wait_visibility(); self.focus_force()
    def _save(self,a,v,tax,ref):
        ...
        self.result=(account,bool(v.get()),tt,rr); self.destroy()
...
class App(tk.Tk):
    ...
    def _build_transaction_tab(self, tab, direction: str):
        ...
        flt = ttk.Combobox(
            ...
            values=["All", "Recurring", "Unassigned", "Ambiguous", "Assigned", "VAT", "No VAT", "Selected for export", "Not selected"],
        ...
        ttk.Button(actions, text="Toggle export selection", command=lambda d=direction: self.toggle_include_selected(d)).pack(side="left", padx=3)
        ttk.Button(actions, text="Select All", command=lambda d=direction: self.set_all_export(d, True)).pack(side="left", padx=(12, 3))
        ttk.Button(actions, text="Clear All", command=lambda d=direction: self.set_all_export(d, False)).pack(side="left", padx=3)
        ...
        cols = ("inc", "date", "details", "amount", "key", "repeat", "account", "vat", "rule", "status")
        tree = ttk.Treeview(tab, columns=cols, show="headings", selectmode="extended")
        self.trees[direction] = tree
        labels = {
            "inc": "Export", "date": "Date", "details": "Bank description / reference", "amount": "Current CSV amount",
            "key": "Matched name + number", "repeat": "Recurring", "account": "GL account", "vat": "VAT", "rule": "Rule", "status": "Status"
        }
        widths = {"inc": 58, "date": 88, "details": 330, "amount": 105, "key": 155, "repeat": 70, "account": 100, "vat": 55, "rule": 145, "status": 175}
        for c in cols:
            if c == "date":
                tree.heading(c, text=labels[c], command=lambda d=direction: self.toggle_date_sort(d))
            else:
                tree.heading(c, text=labels[c])
            tree.column(c, width=widths[c], anchor="center" if c == "inc" else ("e" if c == "amount" else "w"), stretch=(c in {"details", "rule"}))
    ...
    def _build_rules(self):
        ...
        cols = ("id", "direction", "name", "mode", "pattern", "account", "vat", "tax", "ref", "priority")
        self.rule_tree = ttk.Treeview(self.rule_tab, columns=cols, show="headings", selectmode="browse")
        labels = {"id":"ID", "direction":"Type", "name":"Rule name", "mode":"Match", "pattern":"Name / number", "account":"GL account", "vat":"VAT", "tax":"Tax type", "ref":"Pastel ref", "priority":"Priority"}
        widths = {"id":45, "direction":75, "name":170, "mode":85, "pattern":220, "account":100, "vat":55, "tax":70, "ref":80, "priority":65}
        for c in cols:
            self.rule_tree.heading(c, text=labels[c]); self.rule_tree.column(c, width=widths[c], stretch=c in {"name", "pattern"})
    ...
    def refresh_transaction_tree(self, direction: str):
        ...
        for idx, t in items:
            hay = normalize_text(f"{t.details} {t.txn_type} {t.match_key} {t.account} {t.rule_name}")
            ...
            if flt == "Not selected" and t.include:
                continue
            tag = "excluded" if not t.include else ("ambiguous" if t.ambiguous else ("assigned" if t.account else "unassigned"))
            repeat = f"x{t.recurring_count}" if t.recurring_count >= 2 else ""
            vals = ("☑" if t.include else "☐", t.txn_date.strftime("%d/%m/%Y"), t.details, f"R {money(t.payment_amount)}", t.match_key, repeat, t.account, "VAT" if t.vat else "No", t.rule_name, t.status)
        ...
        self.summary_vars[direction].set(
            ...
            f"{recurring} recurring ({recurring_groups} groups) • {ambiguous} ambiguous • {vat_count} VAT • Selected total R {money(total)} • Showing {shown}"
        )
    ...
    def manual_assign_selected(self, direction: str):
        ...
        if dlg.result:
            t.account, t.vat, t.tax_type, t.pastel_ref = dlg.result
            t.rule_id = None; t.rule_name = "Manual (this file only)"; t.status = "Manual assignment"; t.manual_override = True; t.ambiguous = False
            t.match_key = smart_key(t.details, t.txn_type)
            self.refresh_transaction_tree(direction)
    ...
    def toggle_vat_selected(self, direction: str):
        ...
        for t in xs:
            t.vat = target; t.tax_type = default_tax if target else 0; t.manual_override = True
            if t.account:
    ...
    def clear_assignment_selected(self, direction: str):
        ...
        if not xs:
            return
        for t in xs:
            t.rule_id = None; t.rule_name = ""; t.account = ""; t.vat = False; t.tax_type = 0
            t.status = f"Recurring x{t.recurring_count} • unassigned" if t.recurring_count >= 2 else "Unassigned"
            t.manual_override = True; t.ambiguous = False
        self.refresh_transaction_tree(direction)
        ...
        for parent in sorted(groups, key=normalize_text):
            ...
            for key in sorted(children, key=normalize_text):
                ...
                for idx in idxs:
                    ...
                    txn_tag = "excluded" if not t.include else ("ambiguous" if t.ambiguous else ("assigned" if t.account else "unassigned"))
                    tree.insert(
                        kid, "end", iid=tiid,
    ...
    def refresh_rules(self):
        ...
        for r in self.store.all_rules():
            self.rule_tree.insert("", "end", iid=str(r.id), values=(r.id, "Receipt" if r.direction == "RECEIPT" else "Payment", r.name, r.mode, r.pattern, r.account, "Yes" if r.vat else "No", r.tax_type if r.vat else 0, r.pastel_ref, r.priority))
    ...
    def export_rules(self):
        ...
        with open(p, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f); w.writerow(["direction", "name", "mode", "pattern", "account", "vat", "tax_type", "pastel_ref", "priority", "enabled"])
            for r in self.store.all_rules():
                w.writerow([r.direction, r.name, r.mode, r.pattern, r.account, int(r.vat), r.tax_type, r.pastel_ref, r.priority, int(r.enabled)])
            ...
            with open(p, newline="", encoding="utf-8-sig") as f:
                for row in csv.DictReader(f):
                    ...
                    if direction not in self.DIRECTIONS:
                        direction = "PAYMENT"
                    r = Rule(None, row.get("name", "").strip(), row.get("mode", "SMART").strip(), row.get("pattern", "").strip(), row.get("account", "").strip(), row.get("vat", "0").strip() in {"1", "true", "True", "yes", "Yes"}, int(row.get("tax_type", "0") or 0), row.get("pastel_ref", "").strip(), int(row.get("priority", "100") or 100), row.get("enabled", "1").strip() not in {"0", "false", "False"}, direction)
    ...
    def _write_review(self, path: Path, txns: List[Transaction], settings: Dict[str, str], direction: str):
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow(["Source row", "Date", "Bank type", "Bank description/reference", "Smart/matched key", "Recurring seen count", "Matched rule", "GL account", "Cash Book bank GL (Pastel column L)", "VAT", "Tax type", "Gross amount", "VAT amount", "Pastel reference", "Pastel period", "Selected for export"])
            ...
            for t in txns:
                ...
                w.writerow([t.row_no, t.txn_date.strftime("%d/%m/%Y"), t.txn_type, t.details, t.match_key, t.recurring_count, t.rule_name, t.account, effective_contra(t, settings), "VAT" if t.vat else "No VAT", tax, money(t.payment_amount), money(calculate_tax(t.payment_amount, t.vat, rate)), t.pastel_ref, period_for_date(t.txn_date, fiscal), "Yes" if t.include else "No"])
...
def self_test(statement_path: Optional[str] = None) -> str:
    ...
    out = ["Core payment/receipt rules, single cash-book bank GL, selection-only export, VAT and recurrence tests passed."]
    if statement_path:
