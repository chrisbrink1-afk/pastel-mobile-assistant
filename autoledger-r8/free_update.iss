[Setup]
; R8 UPDATE package: deliberately uses the same AppId and install/data paths as R6.
...
OutputDir=output
OutputBaseFilename=AUTOLEDGER_Free_v2.2.5_R8_UPDATE_from_R6_TEST_Setup
Compression=lzma2
...
begin
  Result :=
    ...
    FileExists(ExpandConstant('{localappdata}\Programs\AUTOLEDGER Free\AUTOLEDGER Free v2.2.5 R8 TEST.exe'));
end;
...
begin
  ...
  if not Result then
    MsgBox('This is the AUTOLEDGER Free R8 UPDATE package, but an existing R6/R8 Free installation was not found.' + #13#10 + #13#10 +
