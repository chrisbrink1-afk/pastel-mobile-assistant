...
# R8 and later updates continue accepting the existing R6 entitlement format.
PREFIX = "ALP225R6"
PRODUCT = "AUTOLEDGER"
EDITION = "PRO"
...
def verify_rsa_sha256(message: bytes, signature: bytes, public_n: int = PUBLIC_N, public_e: int = PUBLIC_E) -> bool:
    k = (public_n.bit_length() + 7) // 8
    if len(signature) != k: return False
    sig_int = int.from_bytes(signature, "big")
    if sig_int >= public_n: return False
...
def decode_and_verify_key(key: str) -> dict:
    ...
    if len(parts) != 3 or parts[0] != PREFIX:
        raise ValueError("This is not a valid AUTOLEDGER Pro licence key.")
    payload_bytes = _b64d(parts[1]); signature = _b64d(parts[2])
    if not verify_rsa_sha256(payload_bytes, signature):
    ...
    # v2.2.5 R6 entitlement keys remain permanent across R8+ software updates.
    if payload.get("version") != PRODUCT_VERSION:
        raise ValueError("This licence was issued for a different AUTOLEDGER product version.")
